...
Uncheck the backup drive in Preferences > Catalog > Devices > Disks > Show Info > Contents. Then rescan the catalog by clicking the curved arrow button at the bottom of the catalog or by activating QS with control-space (or Command-Space or whatever you use) and then typing <kbd>⌘</kbd><kbd>R</kbd>. Also make sure Find All Applications is not checked in the catalog, that scans every connected drive.
